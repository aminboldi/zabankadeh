...
export class AttendanceService {
  ...
  async list(sessionId: string, user?: AuthUser): Promise<AttendanceEntry[]> {
    const tenantId = await this.tenantId();
    await this.assertSession(sessionId, tenantId, user);
  ...
  async record(sessionId: string, enrollmentId: string, input: { status?: AttendanceEntry["status"]; note?: string }, user?: AuthUser): Promise<AttendanceEntry> {
    ...
    const entries = await this.list(sessionId, user); const updated = entries.find((entry) => entry.enrollmentId === enrollmentId);
    if (!updated) throw new ConflictException("Attendance was recorded but could not be loaded");
    return updated;
  }

  private async assertSession(sessionId: string, tenantId: string, user?: AuthUser) { const result = await this.db.query<{ branch_id: string }>("select c.branch_id from class_sessions cs join classes c on c.id = cs.class_id and c.tenant_id = cs.tenant_id where cs.id = $1 and cs.tenant_id = $2", [sessionId, tenantId]); if (!result.rows[0]) throw new NotFoundException("Session was not found"); assertBranchAllowed(user, result.rows[0].branch_id); }
