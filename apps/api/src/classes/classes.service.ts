...
export class ClassesService {
  ...
  async list(user?: AuthUser): Promise<ClassSummary[]> {
    ...
    const result = await this.db.query<ClassSummary & { class_code: string; level: string; language: "en" | "de"; age_band: "child" | "teen" | "adult"; branch_name: string; instructor_name: string | null; room_name: string | null; capacity: number; fee_rials: string; class_type: "in_person" | "online" | "hybrid"; status: string }>(
      `select c.id, c.code class_code, pl.name_fa level, pr.language, pr.age_band, b.name_fa branch_name,
       ...
       ${branchFilter} order by c.status, c.code`, params);
  ...
  async options(user?: AuthUser): Promise<ClassOptions> {
    const tenantId = await this.tenantId();
    const branchIds = managedBranchIds(user);
    const branchParams: unknown[] = [tenantId];
    const branchFilter = branchIds ? `and id = any($${branchParams.push(branchIds)}::uuid[])` : "";
    const roomParams: unknown[] = [tenantId];
    const roomFilter = branchIds ? `and branch_id = any($${roomParams.push(branchIds)}::uuid[])` : "";
    const [branches, levels, rooms, instructors, terms] = await Promise.all([
      this.db.query<{ id: string; name: string }>(`select id, name_fa name from branches where tenant_id = $1 and status = 'active' ${branchFilter} order by name_fa`, branchParams),
      ...
      this.db.query<{ id: string; name: string; branch_id: string }>(`select id, name, branch_id from rooms where tenant_id = $1 ${roomFilter} order by name`, roomParams),
  ...
  async create(input: CreateClassInput, user?: AuthUser): Promise<ClassSummary> {
    ...
    const tenantId = await this.tenantId();
    await this.assertBranch(input.branchId, tenantId, user);
    try {
      ...
      const classes = await this.list(user);
  ...
  private async assertBranch(branchId: string, tenantId: string, user?: AuthUser) { const result = await this.db.query<{ id: string }>("select id from branches where id = $1 and tenant_id = $2", [branchId, tenantId]); if (!result.rows[0]) throw new BadRequestException("Branch was not found"); assertBranchAllowed(user, branchId); }
}
