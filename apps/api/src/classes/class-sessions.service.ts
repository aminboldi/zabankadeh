...
import { assertBranchAllowed, managedBranchIds } from "../auth/branch-scope";
...
export class ClassSessionsService {
  ...
  async list(user?: AuthUser): Promise<ClassSessionSummary[]> {
    ...
    const branchIds = managedBranchIds(user);
    const params: unknown[] = [process.env.TENANT_SLUG ?? "demo"];
    const visibility = teacherOnly ? `and p_teacher.mobile = $${params.push(user?.mobile)}` : "";
    const branchFilter = branchIds ? `and c.branch_id = any($${params.push(branchIds)}::uuid[])` : "";
    const result = await this.db.query<{ id: string; class_id: string; class_code: string; level: string; language: "en" | "de"; starts_at: Date; ends_at: Date; instructor_id: string | null; room_id: string | null; instructor_name: string | null; room_name: string | null; status: string; recurrence_rule: "none" | "weekly"; meeting_provider: "none" | "google_meet" | "skyroom" | "other"; meeting_url: string | null }>(
      `select cs.id, cs.class_id, c.code class_code, pl.name_fa level, pr.language, cs.starts_at, cs.ends_at,
       ...
       ${visibility} ${branchFilter} order by cs.starts_at limit 200`, params);
  ...
  async create(input: CreateClassSessionInput, user?: AuthUser): Promise<ClassSessionSummary> {
    ...
    const tenantId = await this.tenantId(); await this.assertClass(input.classId, tenantId, user); const recurrenceRule = input.recurrenceRule ?? "none"; const occurrences = recurrenceRule === "weekly" ? Math.min(Math.max(input.occurrences ?? 1, 1), 52) : 1;
    ...
    const sessions = await this.list(user); const created = sessions.find((session) => session.id === result);
    if (!created) throw new ConflictException("Session was created but could not be loaded");
    return created;
  }
  async update(id: string, input: CreateClassSessionInput, user?: AuthUser): Promise<ClassSessionSummary> {
    if (!input.classId || !input.startsAt || !input.endsAt) throw new BadRequestException("Class, start time, and end time are required");
    ...
    await this.assertSession(id, tenantId, user); await this.assertClass(input.classId, tenantId, user);
    ...
    const result = await this.db.query<{ id: string }>("update class_sessions set class_id = $1, instructor_id = $2, room_id = $3, starts_at = $4, ends_at = $5, meeting_provider = $6, meeting_url = $7, status = $8 where id = $9 and tenant_id = $10 returning id", [input.classId, input.instructorId || null, input.roomId || null, starts, ends, input.meetingProvider ?? "none", input.meetingUrl?.trim() || null, input.status ?? "scheduled", id, tenantId]); if (!result.rows[0]) throw new ConflictException("Session was not found"); const session = (await this.list(user)).find((item) => item.id === id); if (!session) throw new ConflictException("Session was updated but could not be loaded"); return session;
  }
  async cancel(id: string, user?: AuthUser): Promise<ClassSessionSummary> { const tenantId = await this.tenantId(); await this.assertSession(id, tenantId, user); const result = await this.db.query("update class_sessions set status = 'cancelled' where id = $1 and tenant_id = $2 returning id", [id, tenantId]); if (!result.rows[0]) throw new ConflictException("Session was not found"); const session = (await this.list(user)).find((item) => item.id === id); if (!session) throw new ConflictException("Session was cancelled but could not be loaded"); return session; }
  private async assertClass(classId: string, tenantId: string, user?: AuthUser) { const result = await this.db.query<{ branch_id: string }>("select branch_id from classes where id = $1 and tenant_id = $2", [classId, tenantId]); if (!result.rows[0]) throw new BadRequestException("Class was not found"); assertBranchAllowed(user, result.rows[0].branch_id); }
  private async assertSession(id: string, tenantId: string, user?: AuthUser) { const result = await this.db.query<{ branch_id: string }>("select c.branch_id from class_sessions cs join classes c on c.id = cs.class_id and c.tenant_id = cs.tenant_id where cs.id = $1 and cs.tenant_id = $2", [id, tenantId]); if (!result.rows[0]) throw new BadRequestException("Session was not found"); assertBranchAllowed(user, result.rows[0].branch_id); }
