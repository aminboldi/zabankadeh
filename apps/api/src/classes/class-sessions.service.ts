...
export class ClassSessionsService {
  ...
  async list(user?: AuthUser): Promise<ClassSessionSummary[]> {
    ...
    const result = await this.db.query<{ id: string; class_id: string; class_code: string; level: string; language: "en" | "de"; starts_at: Date; ends_at: Date; instructor_name: string | null; room_name: string | null; status: string; recurrence_rule: "none" | "weekly"; meeting_provider: "none" | "google_meet" | "skyroom" | "other"; meeting_url: string | null }>(
    ...
    return result.rows.map((row) => ({ id: row.id, classId: row.class_id, classCode: row.class_code, level: row.level, language: row.language, startsAt: row.starts_at.toISOString(), endsAt: row.ends_at.toISOString(), instructorName: row.instructor_name, roomName: row.room_name, status: row.status, recurrenceRule: row.recurrence_rule, meetingProvider: row.meeting_provider, meetingUrl: row.meeting_url }));
  }
