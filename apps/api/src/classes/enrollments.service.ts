...
import { assertBranchAllowed } from "../auth/branch-scope";

@Injectable()
export class EnrollmentsService {
  constructor(private readonly db: DatabaseService) {}

  async list(classId: string, user?: AuthUser): Promise<ClassEnrollmentSummary[]> {
    const tenantId = await this.tenantId();
    await this.assertClass(classId, tenantId, user);
  ...
  async create(classId: string, input: { studentId?: string; status?: ClassEnrollmentSummary["status"] }, user?: AuthUser): Promise<ClassEnrollmentSummary> {
    ...
    try {
      ...
      const enrollments = await this.list(classId, user); const created = enrollments.find((item) => item.id === result.rows[0].id);
  ...
  async update(id: string, status: ClassEnrollmentSummary["status"], user?: AuthUser): Promise<ClassEnrollmentSummary> {
    ...
    const tenantId = await this.tenantId();
    const existing = await this.db.query<{ class_id: string; branch_id: string }>("select e.class_id, c.branch_id from enrollments e join classes c on c.id = e.class_id and c.tenant_id = e.tenant_id where e.id = $1 and e.tenant_id = $2", [id, tenantId]);
    if (!existing.rows[0]) throw new NotFoundException("Enrollment was not found");
    assertBranchAllowed(user, existing.rows[0].branch_id);
    ...
    const enrollments = await this.list(result.rows[0].class_id, user); const updated = enrollments.find((item) => item.id === id);
    if (!updated) throw new NotFoundException("Enrollment was not found");
    return updated;
  }

  private async assertClass(classId: string, tenantId: string, user?: AuthUser) { const result = await this.db.query<{ branch_id: string }>("select branch_id from classes where id = $1 and tenant_id = $2", [classId, tenantId]); if (!result.rows[0]) throw new NotFoundException("Class was not found"); assertBranchAllowed(user, result.rows[0].branch_id); }
